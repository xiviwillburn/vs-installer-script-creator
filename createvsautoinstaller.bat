...
for /F "tokens=* delims=-" %%i in (configFile) do (
    echo | set /p =%%i >>%myfile%
)
...
echo winget install --id Microsoft.VisualStudio.2022.Community --override "--passive --config %%configPath%%" --accept-package-agreements>>%myfile%
